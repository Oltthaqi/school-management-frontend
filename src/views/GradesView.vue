      ...
      <Card>
        <Table>
          <template #header>
            ...
            <th v-if="authStore.isTeacher || authStore.isAdmin" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
          </template>
          ...
          <tr v-for="grade in grades" :key="grade.id">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
              <span class="font-semibold text-green-600">{{ grade.gradeValue }}</span>
            </td>
            ...
            <td v-if="authStore.isTeacher || authStore.isAdmin" class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
    ...
    >
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <p class="mt-1 text-sm text-gray-900">{{ getStudentName(viewingGrade) }}</p>
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700">Course</label>
          <p class="mt-1 text-sm text-gray-900">{{ getCourseName(viewingGrade) }}</p>
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700">Grade Value</label>
          <p class="mt-1 text-sm text-gray-900">{{ viewingGrade.gradeValue }}</p>
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700">Letter Grade</label>
          <p class="mt-1 text-sm text-gray-900">{{ viewingGrade.letterGrade }}</p>
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700">Graded Date</label>
          <p class="mt-1 text-sm text-gray-900">{{ formatDate(viewingGrade.gradedAt) }}</p>
        </div>
        <div class="col-span-2">
          <label class="block text-sm font-medium text-gray-700">Comments</label>
          <p class="mt-1 text-sm text-gray-900">{{ viewingGrade.comments || 'No comments' }}</p>
        </div>
      </div>
    </DetailModal>
